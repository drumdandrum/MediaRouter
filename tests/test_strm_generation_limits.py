...
class LiveM3uGenerationLimitTests(unittest.TestCase):
    ...
    def test_custom_limit_applies_to_dry_run_and_generate_without_provider_urls(self):
        ...
        self.assertIn('media-router-id="channel_', content)
        self.assertIn('tvg-id="tvg-', content)
        self.assertNotIn('tvg-id="mr:', content)
        self.assertIn("?mr_catalog_id=channel_", content)
