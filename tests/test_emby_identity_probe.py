...
def test_identity_projection_redacts_secrets_and_preserves_markers():
    projected = probe._identity_projection({
        "Id": "15747",
        "Name": "Probe Channel",
        "Path": "http://router/r/live/channel-123?api_key=secret",
    ...
    assert projected["ProviderIds"] == {"TvgId": "mr:channel-123"}
    assert projected["MediaSources"][0]["Id"] == "source-1"
    assert "Token" not in projected["MediaSources"][0]
    assert projected["media_router_identity_detected"]["catalog_item_id"] == "channel-123"


def test_capture_uses_get_only_surfaces_and_reports_missing_session_marker(monkeypatch):
    calls: list[str] = []
    ...
    def fake_request(path, settings):
        ...
        if path.startswith("/LiveTv/Channels"):
            return {"Items": [{
                "Id": "15747", "Name": "Probe Channel", "Type": "TvChannel",
                "ProviderIds": {}, "MediaSources": [{"Id": "source-1"}],
            }]}
        if path == "/Sessions":
            return [{
                "Id": "session-1", "UserId": "user-1",
                "NowPlayingItem": {"Id": "15747", "Name": "Probe Channel", "Type": "TvChannel"},
                "PlayState": {"MediaSourceId": "source-1", "PlaySessionId": "play-1"},
            }]
    ...
    assert payload["interpretation"]["channel_count"] == 1
