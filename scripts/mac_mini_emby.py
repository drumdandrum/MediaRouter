...
def validate_original_inspect(data: dict, expected_id: str = ORIGINAL_ID) -> None:
    if data.get("Id") != expected_id or data.get("Name") != f"/{CONTAINER}":
...
def validate_replacement_inspect(data: dict, volume_name: str) -> None:
    validate_volume_name(volume_name)
    ...
    for target in ("/media-router-test/movies", "/media-router-test/series"):
        if by_target[target].get("RW"):
    ...
    with tempfile.TemporaryDirectory(prefix="emby-backup-check-") as temp:
        ...
        for db in sorted(root.rglob("*.db")):
            try:
                ...
                results[db.name] = "unavailable"
    if any(value not in {"ok", "unavailable"} for value in results.values()):
...
def main() -> None:
    ...
    replacement.add_argument("path"); replacement.add_argument("volume")
    ...
    if args.command == "validate-compose":
        ...
        validate_original_inspect(json.loads(Path(args.path).read_text()), args.expected_id)
    elif args.command == "validate-replacement-inspect":
        validate_replacement_inspect(json.loads(Path(args.path).read_text()), args.volume)
