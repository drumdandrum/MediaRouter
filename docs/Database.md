...
`emby_channel_mappings` is an additive channel crosswalk keyed by Emby server/integration ID and TvChannel ItemId. It stores an optional MediaSourceId, display name, nullable Media Router catalog item, mapping source (`manual`, `automatic_marker`, `automatic_title`, `automatic_placement_title`, or `unmapped`), and creation/update timestamps. Live M3U output carries the catalog ID in a dedicated `media-router-id` attribute and deterministic runtime URL. Exact unique active-placement display-title matching is a refresh-time compatibility fallback when Emby discards those markers; playback never binds by title. Playback resolution prioritizes manual mappings, automatic-marker mappings, other exact ItemId mappings, then exact MediaSourceId mappings; mapping rows never consume capacity themselves.

`emby_vod_item_mappings` is a separate additive adapter-local crosswalk for movies and episodes. Its exact primary key is `(integration_id, emby_item_id)`; rows contain an optional diagnostic MediaSourceId, required catalog item, matching VOD media type, manual mapping source, and timestamps. A reverse diagnostic index covers `(catalog_item_id, integration_id)`. A foreign key removes a mapping if its catalog item is deleted, but mapping writes and deletes never mutate catalog, sources, outputs, reservations, or bindings. There is no backfill, trigger, title/year authority, STRM-file inspection, or automatic VOD mapping application.

When a resolved Emby session first appears without a binding, Broker may atomically adopt exactly one recent, unbound provisional reservation for the same catalog item and media type. Adoption replaces the provisional request identity and aliases with the hashed Emby explicit-session identity; reservation ID, source, account, location, and capacity consumption remain unchanged. Zero, ambiguous, stale, terminal, or already-bound candidates fall back to normal Broker allocation.
