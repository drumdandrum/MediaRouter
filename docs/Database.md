...
When a resolved Emby session first appears without a binding, Broker may atomically adopt exactly one recent, unbound provisional reservation for the same catalog item and media type. Adoption replaces only the request identity with the hashed Emby explicit-session identity; reservation ID, source, account, and capacity consumption remain unchanged. Zero, ambiguous, stale, terminal, or already-bound candidates fall back to normal Broker allocation.
