...
| `GET` | `/api/integrations/emby/vod-item-mappings` | Lists exact, server-scoped movie and episode ItemId mappings. |
| `GET` | `/api/integrations/emby/vod-item-mappings/{server_id}/{item_id}` | Reads one exact VOD ItemId mapping. |
| `PUT` | `/api/integrations/emby/vod-item-mappings/{server_id}/{item_id}` | Upserts one exact movie or episode mapping. |
| `DELETE` | `/api/integrations/emby/vod-item-mappings/{server_id}/{item_id}` | Deletes exactly one VOD mapping without changing catalog or playback state. |
...
VOD mapping bodies use `catalog_id`, `media_type` (`movie` or `episode`), and an optional diagnostic `emby_media_source_id`. The exact `(server_id,item_id)` key is an explicit upsert contract. Catalog existence and exact media-type compatibility are required; channel and series targets are rejected. These adapter-local crosswalks contain no paths, source locators, credentials, tokens, or STRM content. Deletion affects future identity resolution but does not terminate an already-bound playback; that binding retains its previously resolved identity through normal release. No bulk or automatic VOD apply API exists.
...
Every generated live entry carries the stable Media Router catalog identity in the dedicated `media-router-id` attribute, the deterministic runtime path, and the `mr_catalog_id` URL marker. `media-router-id` is deliberately separate from editorial `tvg-id`, so XMLTV identity is preserved. During channel refresh the Emby adapter checks preserved M3U metadata, runtime paths, ProviderIds, media-source IDs, and tuner/source IDs. If no marker exists, an exact normalized canonical-title match is accepted only when the name is unique in both lineups and has no manual or marker conflict. As a compatibility fallback for Emby versions that discard imported markers, refresh can also match an exact active placement display title when the Emby name is unique and the title identifies one distinct catalog item. Titles are HTML-unescaped and whitespace-normalized before comparison; no fuzzy matching is used.
...
Placement-title matching is only a refresh-time compatibility mechanism that creates an exact persisted ItemId mapping. Playback never binds by title; it uses the persisted mapping and other durable identifiers.
