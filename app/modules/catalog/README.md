...
An observed import must also provide a persistent opaque `source_feed_id` UUID. The
UUID belongs to the configured logical feed, not to a catalog item, URL, path, or
playlist position. Imports without a safe UUID continue normally but are not
observed.
...
outside this observation phase.
